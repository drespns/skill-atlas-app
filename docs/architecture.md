...
**App móvil SkillAtlas Gastos (`mobile/`):** monorepo **pnpm** con **`packages/expense-core`** (dominio compartido: `ExpenseTrackerState` v2, merge remoto/local, formato EUR, deudas, escenarios, cifrado E2E) y app **Expo + React Native** (Android primero). Sync: mismo scope **`tools_expense_tracker`** en `user_client_state`; caché local en AsyncStorage. MVP: login Supabase (email/contraseña), home mes (KPIs, donut por categoría, lista por día), alta rápida gasto/ingreso, ajustes (toggle sync, enlace web), desbloqueo E2E si la nube está cifrada. Build Play Store: **EAS** (`mobile/eas.json`). Scripts raíz: `pnpm mobile`, `pnpm mobile:android`. Ver **`mobile/README.md`**.
