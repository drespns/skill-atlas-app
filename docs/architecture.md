...
**Responsive (tablet / portátil):** el contenido principal (`.app-main-shell` en `global.css`) usa relleno horizontal **`clamp(1rem, 4vw, 2.5rem)`** (`--app-shell-inline`) para que no quede pegado a los bordes en anchos intermedios. La cabecera (`AppHeader.astro`) concentra el padding en la fila interna `max-w-screen-2xl` (`px-4 sm:px-6 lg:px-8`) y reduce un poco el padding de los enlaces de nav en `md`. La landing (`/`) comparte el mismo tipo de márgenes en el contenedor `max-w-screen-2xl`; el carrusel de facetas vuelve a ser **compacto** (tarjetas con ancho fijo + snap, no pantalla completa). La página `/pricing` añade padding horizontal al bloque interior. **Menú de usuario (avatar):** solo **Ajustes** y **Cerrar sesión** (se quitó “Perfil público” por redundar con Ajustes).

**SEO / metadatos:** `AppShell.astro` incluye `SiteMeta.astro` (description, canonical, Open Graph, Twitter `summary_large_image`, `theme-color`, favicon). Valores por defecto en `src/config/site-meta.ts`; imagen OG por defecto `public/og/og-default.svg`. **`astro.config.mjs`** admite `site: process.env.PUBLIC_SITE_URL` para URLs absolutas en OG (documentado en `.env.example`). Las páginas pueden pasar `description`, `ogImage` y `noindex` como props del layout.

**Footer:** bloque visual con gradientes, halos y tarjeta de marca; **borde superior con `clip-path`** en forma de V (`polygon(50% 17%, 100% 0, …)`); enlaces en píldoras; fila “Hecho con” con iconos de stack (sin enlace duplicado a “código abierto” / GitHub junto al stack).

**Banner global (`AppGlobalBanner.astro`):** estética alineada con el footer (halos indigo/violeta, fondo en capas, anillo/sombra); texto y versión desde `src/config/banner.ts`.

**Carrusel de facetas (landing):** desplazamiento **solo dentro del carril** (`track.scrollTo`, sin `scrollIntoView` para no desplazar la página); arrastre con `requestAnimationFrame`, `snap-proximity`, `overscroll-x-contain`; avance automático cada ~5,5s con **relleno de progreso** en el punto activo; puntos **extremos no clicables**; transición de tarjetas desactivada mientras `is-dragging`; respeta `prefers-reduced-motion`.
...
- **Idioma:** `lang` en prefs es **`es` \| `en`** (normalizado en `core/prefs.ts`); i18next arranca con `loadPrefs().lang` (`i18n-bootstrap.ts`). El selector en cabecera es un **popover al hover** (y clic/táctil) bajo la bandera, mismo patrón visual que el menú del logo (`data-lang-wrap` / `lang-picker-popover.ts`, opciones en `lib/lang-picker-options.ts`, banderas en `public/icons/flags/*`). Filas sin traducción activa quedan deshabilitadas. **`showLangSelector`** en Preferencias muestra el icono en el resto de rutas; en **`/`** y **`/login`** el selector se muestra **siempre** para invitados y sesión iniciada. `en.json` y `es.json` alimentan `data-i18n` en el shell.
...
- **`settingsActiveSection`**: última sección de Ajustes abierta. IDs de panel (hash `#…`): `prefs`, `shortcuts`, `portfolio-profile`, `portfolio-links`, `portfolio-display`, `portfolio-presentation`, `qa` (lista canónica en `SETTINGS_PANEL_IDS` en `src/scripts/core/prefs.ts`). El bloque de **CV público** vive dentro de `#portfolio-links`. El hash antiguo `#cv-public` se **redirige** a `#portfolio-links` y hace scroll al bloque CV. Si la URL o prefs guardadas usan el prefijo antiguo `#classic-*`, el cliente **normaliza** a los ids nuevos (`migrateSettingsPanelHashFragment`, `replaceState` en `settings/settings-classic-ui.ts`).
