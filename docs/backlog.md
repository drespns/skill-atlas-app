...
**Producto activo:** cuaderno de gastos (`/tools/expense-tracker`). Entrada `/` = landing demo + login; sesión → cuaderno. Cabecera «Finanzas»; legacy fuera de barra.

- **Home de entrada:** split landing (demo KPIs/charts) + tarjeta de login; `/login` reenvía a `/` conservando hash OAuth.
