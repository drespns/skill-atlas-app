-- SkillAtlas SaaS - 016
-- Portada por proyecto: ruta en Storage (pública) + campo cover_image_path; RPCs exponen coverImagePath.
--
-- Prerequisites: saas-015 aplicado (misma forma de RPCs que 014+015).

ALTER TABLE public.projects
  ADD COLUMN IF NOT EXISTS cover_image_path text;

COMMENT ON COLUMN public.projects.cover_image_path IS
'Ruta en bucket project_covers (p.ej. userId/projectId/cover.webp); URL pública = /storage/v1/object/public/project_covers/...';
...
DROP POLICY IF EXISTS "project_covers_insert_own" ON storage.objects;
...
DROP POLICY IF EXISTS "project_covers_update_own" ON storage.objects;
...
DROP POLICY IF EXISTS "project_covers_delete_own" ON storage.objects;
