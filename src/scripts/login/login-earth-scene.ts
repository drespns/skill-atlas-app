...
/** Mismo tono base que la columna del login (evita halo al componer alpha en el canvas). */
function loginEarthClearHex(): number {
  return document.documentElement.classList.contains("dark") ? 0x030712 : 0xf8fafc;
}

export function mountLoginEarthScene(canvas: HTMLCanvasElement, container: HTMLElement): () => void {
  ...
  renderer.setClearColor(loginEarthClearHex(), 1);
  renderer.outputColorSpace = THREE.SRGBColorSpace;
  renderer.toneMapping = THREE.ACESFilmicToneMapping;
  renderer.toneMappingExposure = 1.0;

  const syncClearToTheme = () => {
    renderer.setClearColor(loginEarthClearHex(), 1);
  };
  const themeObserver = new MutationObserver(() => syncClearToTheme());
  ...
  const loadImage = (url: string, onOk: (t: THREE.Texture) => void, label: string) => {
    loader.load(
      url,
      (t) => {
  ...
  const rim = new THREE.PointLight(0x88ccff, 0.35, 50);
