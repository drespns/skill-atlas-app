...
function ensureRoot() {
  let root = document.querySelector<HTMLElement>("[data-onboarding-v2-root]");
  if (!root) {
    root = document.createElement("div");
    root.dataset.onboardingV2Root = "1";
...
function render(stepIdx: number) {
  ...
  overlay.innerHTML = `
    <div data-v2-backdrop class="absolute inset-0 bg-black/50 pointer-events-none transition-opacity duration-150"></div>
    <div data-v2-hole class="absolute rounded-xl ring-4 ring-indigo-400/60 pointer-events-none" style="box-shadow: 0 0 0 9999px rgba(0,0,0,0.45);"></div>
    <div data-v2-pill-wrap class="hidden fixed z-1201 left-3 bottom-3 pointer-events-auto items-center gap-2">
      <button
        ...
        aria-label="${esc(tt("onboardingV2.open", "Onboarding · Abrir"))}"
        title="${esc(tt("onboardingV2.open", "Onboarding · Abrir"))}"
      >
        <span class="header-settings-rainbow__inner px-3 py-2 text-sm font-semibold whitespace-nowrap">
          ${esc(tt("onboardingV2.open", "Onboarding · Abrir"))}
        </span>
      </button>
      <button
        type="button"
    ...
    <div data-v2-tip class="absolute w-[min(92vw,34rem)] rounded-2xl border border-gray-200/80 dark:border-gray-800 bg-white dark:bg-gray-950 shadow-xl overflow-hidden pointer-events-auto">
      <div data-v2-drag-handle class="px-5 py-4 border-b border-gray-200/80 dark:border-gray-800 flex items-start justify-between gap-4 cursor-move select-none">
        <div class="min-w-0">
          <p class="m-0 text-xs font-semibold text-gray-500 dark:text-gray-400">${esc(tt("onboardingV2.kicker", "Onboarding"))} · ${doneCount}/${STEPS.length}</p>
          <h3 class="m-0 mt-1 text-lg font-semibold text-gray-900 dark:text-gray-100">${esc(tt(step.titleKey, step.titleFallback))}</h3>
        </div>
...
if ((window as any).__skillatlasOnboardingV2Bound !== true) {
  (window as any).__skillatlasOnboardingV2Bound = true;
  ...
  window.addEventListener("skillatlas:onboarding-v2-start", () => startFromBeginning());
}
