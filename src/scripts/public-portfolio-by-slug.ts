...
async function run() {
  ...
  const { data, error } = await supabase.rpc("skillatlas_portfolio_by_public_slug", { p_slug: slug });
  ...
  const techNamesAll = Array.from(
    new Set(projects.flatMap((p) => p.technologyNames ?? [])),
